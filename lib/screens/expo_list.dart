...
import 'package:expo_in/componenets/expo/userDetail.dart';
import 'package:expo_in/design-system/constant.dart';
...
Stream<QuerySnapshot> _asyncProfo(String name, String keyo) =>
...
class ECardList extends StatefulWidget {
  final String name;
  final String keyo;
  const ECardList({Key? key, required this.name, required this.keyo})
      : super(key: key);

  @override
  State<ECardList> createState() => _ECardListState();
}

class _ECardListState extends State<ECardList> {
  late final getData = _asyncProfo(widget.name, widget.keyo);
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasData) {
            snapshot.data.docs.forEach((value) {
              final productTile = UserDetails(
                name: value.data()['key'],
                keyo: value.id,
                parentKey: widget.keyo,
              );
            ...
            tilesList.insert(
              ...
              Padding(
                padding:
                    const EdgeInsets.only(left: 24.0, right: 24.0, bottom: 60),
                child: Column(
